...
public:
    ...
    Term &operator*(Term &obj)
    {
        Term *res = new Term(this->coeff * obj.coeff, this->pow + obj.pow);
        return *res;
    }
};
...
public:
    ...
    Polynomial &operator*(Polynomial &obj)
    {
        ...
        for (int i = 0; i < p1Size; i++)
        {
            ...
            {                
                int newExp = this->terms[i].pow + obj.terms[j].pow;
                bool flag = false;
                for (int k=0;k<p->terms.size();k++){
                    if(newExp == p->terms[k].pow){
                        p->terms[k].coeff += this->terms[i].coeff * obj.terms[j].coeff;
                        flag = true;
                    }
                }
